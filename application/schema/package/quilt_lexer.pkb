CREATE OR REPLACE PACKAGE BODY quilt_lexer AS
    ...
    PROCEDURE initializeMatchers IS
        PROCEDURE appendMatcher(p_matcher quilt_matcher) IS
        BEGIN
            g_matchers.extend();
    ...
    BEGIN
        ...
        appendMatcher(NEW quilt_MatchWhitespace());
        --
    ...
    ----------------------------------------------------------------------------
    FUNCTION nextTokenImpl RETURN quilt_token IS
        l_Result quilt_token;
    BEGIN
        IF eof THEN
            l_Result := NEW quilt_token(tk_EOF);
        ELSE
    ...
    ----------------------------------------------------------------------------
    FUNCTION nextToken RETURN quilt_token IS
        l_Result quilt_token;
    BEGIN

CREATE OR REPLACE PACKAGE BODY quilt_log_pkg IS
  ...
  PROCEDURE log_start(p_runid IN NUMBER, p_test_name IN VARCHAR2) IS
  ...
  BEGIN
      INSERT INTO quilt_log
        (sessionid, sid, runid, msg, insert_ts)
        VALUES
          (lint_sessionid, lint_sid, lint_runid, p_msg, systimestamp);
      COMMIT;
  END log_detail;
